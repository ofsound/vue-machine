window.addEventListener('load', function () {
  ...
  function triggerNotes() {
    ...
    let colorPhase = 0
    if (Math.random() > 0.5) colorPhase = 1.43
    ...
    for (let squareIndex = 0; squareIndex < numNotes; squareIndex++) {
      ...
      for (let lightIndex = 0; lightIndex < 36; lightIndex++) {
        ...
        let randomNumber = Math.random() * 0.01

        let numberForLightBetween0and1 = Math.abs(
          Math.sin(lightIndex * (0.045 + randomNumber) + colorPhase),
        )
        ...
        colorsBack.forEach(function (colorBack, colorIndex) {
          ...
          gsap.to(thisLight, {
            ...
            delay: squareStartTime * 0.01 + squareEndTime + 0.02 * (35 - lightIndex),
          })
        })
      }
    }
